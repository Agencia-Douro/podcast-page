...
import DiretorPlaceholder from "@/public/vania-podcast.png";
...
export function DiretorProducao() {
    const t = useTranslations("Podcast.diretorProducao");
    ...
    return (
        <section className="container pt-8 md:pt-10 lg:pt-12 xl:pt-16 overflow-x-hidden">
            <div className="flex flex-col gap-6 md:gap-8 lg:flex-row lg:justify-between lg:gap-12 xl:gap-16 lg:items-stretch min-h-0">
                {/* Imagem - Desktop: aparece primeiro (esquerda) */}
                <div className="hidden lg:flex lg:justify-center lg:items-center lg:min-w-[340px] lg:w-[45%] lg:shrink-0 bg-muted">
                    <div className="relative w-full aspect-4/5 overflow-hidden max-h-full">
                        <div className="absolute inset-x-0 top-0 h-[88%] overflow-hidden">
                            <Image
                                src={siteConfig?.diretorImage || DiretorPlaceholder}
                                alt={t("imageAlt")}
                                fill
                            ...
                            <div className="absolute bottom-4 left-4 right-4 z-10 flex flex-col items-end gap-3">
                                <Link
                                    href={podcastContent?.directorLinkedInUrl || t("linkedInUrl")}
                                    target="_blank"
                                    ...
                                    aria-label={podcastContent?.directorLinkedInLabel || t("linkedInLabel")}
                                >
                ...
                <div className="space-y-4 md:space-y-6 lg:space-y-8 text-left flex flex-col min-w-0 min-h-0">
                    ...
                    <div className="space-y-4 text-justify lg:text-left min-w-0">
                        ...
                        <p className="text-black-muted md:body-18-regular body-16-regular leading-relaxed text-pretty wrap-break-word">{podcastContent?.directorParagraph2 || t("paragraph2")}</p>
                        <p className="text-black-muted md:body-18-regular body-16-regular leading-relaxed text-pretty wrap-break-word">{podcastContent?.directorParagraph3 || t("paragraph3")}</p>
                        <p className="body-16-medium text-black border-l-2 border-brown pl-4 italic text-pretty wrap-break-word lg:hidden">&quot;{podcastContent?.directorQuote || t("quote")}&quot;</p>
                    </div>
